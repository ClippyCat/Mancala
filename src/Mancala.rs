const Board_Size:i8 = 12;
	...
	fn init_board(numRocks:i8) -> Board {
...
// Define a function to handle player input
	...
	fn moveRocks(board: &mut Board, pitI: usize) {
...
fn captures(board: &mut Board, lastI: usize) {
	let numPits = Board_Size as usize / 2 - 1;
	let mut i = lastI;
	let activePlayer = board.activePlayer;
	while board.pits[i] == 1 && i != game_state.player1.mancala_index && i != game_state.player2.mancala_index {
		let opposite = numPits - i;
		let canCapture = board.pits[opposite];
		if i < numPits && activePlayer == 1 || i > numPits && activePlayer == 2 {
			board.pits[i] = 0;
			board.pits[opposite] = 0;
			board.pits[activePlayer.mancala_index()] += canCapture + 1;
		}
		i = (i + Board_Size as usize - 1) % Board_Size as usize;
	}
}

	// Define a function to check the game's status
fn checkStatus(board: &Board) -> GameStatus {
	let numPits = Board_Size as usize / 2 - 1;
	...
	if p1_sum == 0 || p2_sum == 0 {
		...
		if board.pits[numPits] > board.pits[Board_Size as usize - 1] {
			return GameStatus::GameOver(1);
		} else if board.pits[numPits] < board.pits[Board_Size as usize - 1] {
			return GameStatus::GameOver(2);
		} else {
			return GameStatus::GameOver(0);
		}
	} else {
...
fn playGame(mut state: GameState) {
